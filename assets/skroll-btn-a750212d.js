...
<use href="${$}#heart"></use>
    ...
    <li class="info-item">
        ...
        </div>`;T.insertAdjacentHTML("afterbegin",I),j()}function j(){k.classList.add("open"),document.body.style.overflow="hidden",O.addEventListener("click",l),document.addEventListener("mouseup",q),document.addEventListener("keydown",x)}function l(){k.classList.remove("open"),O.removeEventListener("click",l),document.removeEventListener("mouseup",q),document.removeEventListener("keydown",x),T.innerHTML="",document.body.style.overflow="",c.removeEventListener("click",v),c.removeEventListener("click",g)}const q=function(e){!document.getElementById("modal").contains(e.target)&&k.classList.contains("open")&&l()},x=function(e){e.key==="Escape"&&l()},K=e=>{const{_id:t,bodyPart:s,burnedCalories:i,description:o,equipment:n,gifUrl:r,name:d,popularity:u,rating:m,target:f,time:w}=e;return{_id:t,bodyPart:s,burnedCalories:i,description:o,equipment:n,gifUrl:r,name:d,popularity:u,rating:m,target:f,time:w}},v=()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard"),s=[];t!=null&&(s=JSON.parse(t));const i=K(a);s.push(i),localStorage.setItem("favoritesCard",JSON.stringify(s)),b(e)},g=async()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard"),s=JSON.parse(t).filter(i=>i._id!=e);s.length!=0?localStorage.setItem("favoritesCard",JSON.stringify(s)):localStorage.removeItem("favoritesCard"),b(e)},b=e=>{let t=localStorage.getItem("favoritesCard");c.addEventListener("click",v),c.removeEventListener("click",g),c.innerHTML=`Add to favorites ${S}`,t!=null&&JSON.parse(t).forEach(s=>{s._id==e&&(c.innerHTML=`Remove favorite ${S}`,c.removeEventListener("click",v),c.addEventListener("click",g))})},z=100,L=document.querySelector(".skroll-btn"),G=()=>window.scrollY||document.documentElement.scrollTop;window.addEventListener("scroll",()=>{G()>z?L.classList.add("skroll-btn-active"):L.classList.remove("skroll-btn-active")});L.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});export{Q as a,$ as i};
//# sourceMappingURL=skroll-btn-a750212d.js.map
